...
const SOURCE_TYPES = ["river", "pond", "spring"];
...
export default function App() {
  ...
  async function selectLocation(result) {
    ...
    if (result.district) {
      ...
      // Keep the full source collection loaded; the map uses districtFilter for display.
    }
  }
  ...
  const step = Math.min(1, Math.max(0.15, Math.abs(e.deltaY) / 400));

  zoomMap(direction * step, anchor);
}
  function applyFilters(e) {
    e.preventDefault();
    setDistrictFilter(filterDistrict);
    // Filters are applied client-side through filteredSources.
  }
  ...
  // ─── Filter sidebar (reused in both map and table tabs) ──────────────────
      ...
      <aside className="filter-panel">
        ...
        <form onSubmit={applyFilters}>
          <div className="filter-search-box">
            <SearchIcon />
            <input className="filter-search-input" onChange={(e) => setQuery(e.target.value)} placeholder="Search by name, location..." value={query} />
          </div>
          <div className="filter-group">
            <label className="filter-label">State / UT</label>
            <select className="filter-select" value={filterState} onChange={(e) => { setFilterState(e.target.value); setFilterDistrict(""); }}>
        ...
        {isLoggedIn && (
          <div className="filter-user-section">
            ...
            <button className="filter-add-btn" onClick={() => openAddSource()} type="button">+ Add Source</button>
          </div>
        )}
      </aside>
    );
  ...
  if (booting) {
    return (
      <div className="splash">
        <div className="splash-logo"><DropIcon size={52} color="#1645a3" /></div>
  ...
  <img src="/logo.png" alt="JalSrot" className="brand-logo" />
</div>
...
["contact", "Contact & Support"],
  ...
  <div className="home-detail-grid">
    <div className="home-detail-image-wrap">
      <img
        src="/home.jpg"
        alt="Natural water source at Praniya Walla, Uttarakhand"
    ...
    <div className="home-detail-copy">
      ...
      <p>
        The project is being carried out at G.B. Pant Institute of Engineering
        &amp; Technology, Pauri Garhwal, with a focus on understanding,
        documenting and supporting water resources in the region.
      </p>

      <p>
        The work brings together field information, digital mapping and
        technology-based approaches to support better understanding and
          ...
          Learn About the Project
            ...
            <div className="home-focus-section">
              <div className="home-section-heading">
                ...
                <p>Our project focuses on smart management, sustainability and resilience of Himalayan water resources.</p>
              </div>
              <div className="home-focus-grid">
                <div className="home-focus-card">
                  <div className="home-card-icon">💧</div>
                  <h3>Smart Water Resource Management</h3>
                  <p>Exploring approaches toward smarter management of water resources in the Himalayan context.</p>
                </div>
                <div className="home-focus-card">
                  <div className="home-card-icon">🌱</div>
                  <h3>Sustainable Water Resources</h3>
                  <p>Promoting a long-term perspective on the sustainable management of water resources.</p>
                </div>
                <div className="home-focus-card">
                  <div className="home-card-icon">🏔️</div>
                  <h3>Himalayan Communities</h3>
                  <p>Recognizing the importance of sustainable water resources for communities in the Himalayan region.</p>
                </div>
                <div className="home-focus-card">
                  <div className="home-card-icon">🌊</div>
                  <h3>Downstream Resilience</h3>
                  <p>Considering the broader importance of water-resource resilience for downstream regions.</p>
                </div>
              </div>
            </div>
            ...
            <div className="home-bottom-banner">
              <div>
                ...
                <p>Help build awareness and support sustainable water resource management.</p>
              </div>
              <button className="home-primary-btn" onClick={() => goToSection("sources-list")} type="button">
                View Water Sources →
              </button>
            </div>
          </section>
      ...
      {activeSection === "resources" && (
        <div className="main-layout resources-map-layout">
              <FilterSidebar />
              <section className="map-section">
                <div className="map-bar">
                  <form className="map-search-form" onSubmit={searchLocation}>
                    <div className="map-search-wrap">
                      <SearchIcon />
                      <input className="map-search-input" onChange={(e) => setLocationQuery(e.target.value)} placeholder="Search a location" value={locationQuery} />
                    </div>
                  ...
                  {selectedPoint && <button className="map-use-btn" onClick={() => openAddSource()} type="button">Use picked point</button>}
                </div>
                  ...
                  <div className="location-results">
                    ...
                    {locationResults.map((r) => (
                      <button key={r.id} className="location-result-item" onClick={() => selectLocation(r)} type="button">
                        <strong>{r.name}</strong><span>{r.displayName}</span>
                      </button>
                    ))}
                  </div>
                )}
                ...
                <div
                  ...
                  onPointerDown={beginMapDrag} onPointerMove={moveMapDrag}
                  onPointerUp={endMapDrag} onPointerCancel={endMapDrag}
                  onWheel={handleMapWheel} ref={mapRef} role="presentation"
                >
                  ...
                  {projectedSearchedPoint && (
                    <button className="map-pin search-pin" onClick={(e) => e.stopPropagation()} style={projectedSearchedPoint} title={searchedPoint?.name} type="button">
                  ...
                  {projectedSources.map(({ source, left, top }) => {
                    ...
                    return (
                      <button className="map-dot-pin" key={source._id || source.id || source.name}
                        onClick={(e) => { e.stopPropagation(); setSelectedSource(source); }}
                        onPointerDown={(e) => e.stopPropagation()} style={{ left, top }} title={source.name} type="button">
                        <span className="dot-circle" style={{ background: color }} />
                      </button>
                    );
                  })}
                  {selectedPoint && (
                    <button className="map-pin add-pin" onClick={(e) => { e.stopPropagation(); openAddSource(selectedPoint); }} style={projectedSelectedPoint} type="button">
                      <span>+</span>
                    </button>
                  )}
                  <div className="map-legend">
                    {SOURCE_TYPES.filter((t) => t !== "well" && t !== "borewell").map((t) => (
      ...
      {activeSection === "sources-list" && (
        <section className="page-section view-page" id="sources-list">
          <div className="sources-list-header">
            <h2 className="section-heading">Water Sources</h2>
            <p className="section-sub">
              Explore all documented water sources and view their complete details.
            </p>
          </div>

          <div className="sources-table-fullview">
                  ...
                  <tbody>
                    {sources.length === 0 ? (
                      <tr>
                        ...
                        >
                          {sourcesBusy ? "Loading sources..." : "No water sources found."}
                        </td>
                      </tr>
                    ) : (
                      sources.map((source, i) => {
                        const { latitude, longitude } = sourceCoords(source);
                        const color =
                          SOURCE_TYPE_COLORS[source.sourceType] || "#6b7280";
      ...
      {activeSection === "about" && (
        <section className="page-section view-page" id="about">
          ...
          <p className="section-sub">
            ...
            <strong>&ldquo;Smart and Sustainable Water Resource Management for
            Himalayan Communities and Downstream Resilience in Uttarakhand,&rdquo;</strong>{" "}
            focuses on managing water resources sustainably while
            considering the needs of Himalayan communities and downstream resilience.
          </p>
          <p className="section-sub">
            The project is being undertaken at{" "}
            <strong>G.B. Pant Institute of Engineering &amp; Technology, Pauri Garhwal, Uttarakhand,</strong>{" "}
            with financial support from <strong>UCOST, Government of Uttarakhand.</strong>
          </p>
          <p className="section-sub">
            The project is led by <strong>Dr. Pushkar Praveen, Principal Investigator,</strong>{" "}
            with <strong>Dr. Agya Ram Verma</strong> and <strong>Dr. Papendra Kumar</strong> serving as{" "}
            <strong>Co-Principal Investigators.</strong>
          </p>
          <h3 className="about-heading">Project Vision</h3>
          <p className="section-sub">
            To support sustainable and smart water resource management
            approaches that contribute to resilient Himalayan communities
            and strengthen downstream water-resource resilience in Uttarakhand.
          </p>
          <h3 className="about-heading">Why This Project Matters</h3>
          <p className="section-sub">The project title itself identifies two interconnected areas of concern</p>
          <div className="about-info-block">
            <h4>Himalayan Communities</h4>
            <p className="section-sub">
              Water resources are important for life and development in Himalayan communities.
              Sustainable management of these resources supports community resilience.
            </p>
          </div>
          <div className="about-info-block">
            <h4>Downstream Resilience</h4>
            <p className="section-sub">
              Water-resource conditions in Himalayan regions have implications beyond immediate communities.
              The project therefore considers <b>downstream resilience</b> as part of its broader focus areas.
            </p>
          </div>
          <div className="about-info-block">
            <h4>Smart &amp; Sustainable Management</h4>
            <p className="section-sub">
              The project specifically emphasizes <b>&ldquo;Smart and Sustainable Water Resource Management,&rdquo;</b>{" "}
              indicating a focus on approaches that combine improved management with long-term sustainability.
            </p>
          </div>
          <div className="about-objectives-section">
            <h3 className="about-heading">Project Objectives</h3>
            <ul className="about-objectives">
              <li>To explore smart approaches for improved water resource management.</li>
              <li>To consider the water-resource needs and resilience of Himalayan communities.</li>
            </ul>
          </div>
        </section>
      )}
      ...
      {activeSection === "gallery" && (
        <section className="page-section alt view-page" id="gallery">
          ...
          <p className="section-sub">Photos and videos captured on-site while surveying water sources in Pauri Garhwal.</p>
          <div className="gallery-grid">
            {GALLERY_PHOTOS.map((photo) => (
              <div className="gallery-card" key={photo.src}>
                <button className="gallery-image-button" onClick={() => setGalleryZoom(photo)} aria-label={`View ${photo.title}`} type="button">
      ...
      {activeSection === "contributions" && (
        <section className="page-section view-page contributions-page" id="contributions">
          ...
          <p className="section-sub">The project team working towards smart and sustainable water resource management in Uttarakhand.</p>

          <h3 className="contribution-heading">Institute Leadership</h3>
          <div className="leadership-grid">
            <div className="leadership-card">
              <div className="leadership-image"><img src="/director-vk-banga.jpg" alt="Prof. Dr. V. K. Banga" /></div>
              <div className="leadership-details">
                <h4>Prof. (Dr.) V. K. Banga</h4>
                <p>Director</p>
                <p>Govind Ballabh Pant Institute of Engineering and Technology, Pauri Uttarakhand</p>
              </div>
            </div>
            <div className="leadership-card">
              <div className="leadership-image"><img src="/dean-mamta-baunthiyal.jpg" alt="Dr. Mamta Baunthiyal" /></div>
              <div className="leadership-details">
      ...
      {activeSection === "contact" && (
        <section className="page-section alt view-page" id="contact">
          ...
          <p className="section-sub">Questions, feedback, or want to contribute data? Get in touch with us using the contact details below.</p>
          <div className="info-card-row" style={{ marginBottom: 28 }}>
            <div className="info-card">
              <strong>Report an issue</strong>
              <p>
                Found incorrect data, a bug, or a problem with the website? Email us at{" "}
                <a href="mailto:Jalsrotuk@gmail.com">Jalsrotuk@gmail.com</a>{" "}
                with a short description of the issue. A screenshot can also help us understand the problem.
              </p>
            </div>
          </div>
          <h3 className="section-heading" style={{ fontSize: "1.05rem" }}>How to contribute</h3>
          <div className="info-card-row">
            <div className="info-card">
              <strong>Share field photos</strong>
              <p>
                Have a photograph of a water source? Email them to{" "}
                <a href="mailto:Jalsrotuk@gmail.com">Jalsrotuk@gmail.com</a>{" "}
